...
class Case2_MonoClassMonoLabel(unittest.TestCase):
    ...
    def test02_Model_TfidfSvm(self):
        ...
        try:
            ...
            model_name = 'tfidf_svm_mono_class_mono_label_super_documents'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
    ...
    def test03_Model_TfidfGbt(self):
        ...
        try:
            ...
            # Set super documents model
            model_name = 'tfidf_gbt_mono_class_mono_label_super_documents'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
    ...
    def test04_Model_TfidfLgbm(self):
        ...
        try:
            ...
            # Set super documents model
            model_name = 'tfidf_lgbm_mono_class_mono_label_super_documents'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
    ...
    def test05_Model_TfidfDense(self):
        ...
        try:
            ...
            # Set super documents model
            model_name = 'tfidf_dense_mono_class_mono_label_super_documents'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
    ...
    def test11_Model_TfidfSgdc(self):
        ...
        try:
            ...
            # Set super documents model
            model_name = 'tfidf_sgdc_mono_class_mono_label_super_documents'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
    ...
    def test14_Model_TfidfCos(self):
        '''Test of the model TF-IDF/Cos'''
        print('            ------------------ >     Test of the model TF-IDF/Cos     /   Mono-class & Mono-label')
        ...
        try:
            ...
            # Set model
            model_name = 'tfidf_cos_mono_class_mono_label'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
            ...
            # Set super documents model
            model_name = 'tfidf_cos_mono_class_mono_label_super_documents'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
    ...
    def test15_Model_TfidfSuperDocumentsNaive(self):
        '''Test of the model TF-IDF/Super Documents Naive'''
        print('            ------------------ >     Test of the model TF-IDF/Super Documents Naive     /   Mono-class & Mono-label')
        ...
        try:
            ...
            # Set model
            model_name = 'tfidf_super_documents_naive_mono_class_mono_label'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
    ...
    def test16_Model_Aggregation(self):
        ...
        try:
            ...
            list_models = [model_tfidf_svm.ModelTfidfSvm(), model_tfidf_svm.ModelTfidfSvm(), model_tfidf_gbt.ModelTfidfGbt()]
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
            ...
            gbt = model_tfidf_gbt.ModelTfidfGbt()
            list_models = [svm1, svm2, gbt]
            model_name = 'aggregation_mono_class_mono_label'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
            test_model = model_aggregation.ModelAggregation(x_col='preprocessed_text', y_col='y_col', level_save="HIGH",
            ...
            list_models = [svm, model_tfidf_svm.ModelTfidfSvm(), model_tfidf_gbt.ModelTfidfGbt()]
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
            test_model = model_aggregation.ModelAggregation(x_col='preprocessed_text', y_col='y_col', level_save="HIGH",
                                                        list_models=list_models, using_proba=False, aggregation_function='majority_vote',
...
class Case3_MonoClassMultiLabel(unittest.TestCase):
    ...
    def test14_Model_Aggregation(self):
        ...
        try:
            ...
            # Set model function all_predictions and list_models=[model, model]
            model_name = 'aggregation_mono_class_multi_label'
            list_models = [model_tfidf_svm.ModelTfidfSvm(multi_label=True, ), model_tfidf_svm.ModelTfidfSvm(multi_label=True, ), model_tfidf_gbt.ModelTfidfGbt(multi_label=True, )]
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
            test_model = model_aggregation.ModelAggregation(x_col='preprocessed_text', y_col='y_col', level_save="HIGH",
            ...
            # Set model with function all_predictions and list_models=[model_name, model_name]
            model_name = 'aggregation_mono_class_multi_label'
            svm1 = model_tfidf_svm.ModelTfidfSvm(multi_label=True, )
            svm2 = model_tfidf_svm.ModelTfidfSvm(multi_label=True, )
            gbt = model_tfidf_gbt.ModelTfidfGbt(multi_label=True, )
            list_models = [svm1, svm2, gbt]
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
            test_model = model_aggregation.ModelAggregation(x_col='preprocessed_text', y_col='y_col', level_save="HIGH",
            ...
            # Set model with function all_predictions and list_models=[model_name, model]
            model_name = 'aggregation_mono_class_multi_label'
            svm = model_tfidf_svm.ModelTfidfSvm(multi_label=True, )
            list_models = [svm, model_tfidf_svm.ModelTfidfSvm(multi_label=True, ), model_tfidf_gbt.ModelTfidfGbt(multi_label=True, )]
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
            test_model = model_aggregation.ModelAggregation(x_col='preprocessed_text', y_col='y_col', level_save="HIGH",
                                                            list_models=list_models, using_proba=False, aggregation_function='all_predictions',
...
class Case4_MultiClassMonoLabel(unittest.TestCase):
    ...
    def test14_Model_TfidfCos(self):
        '''Test of the model TF-IDF/Cos'''
        print('            ------------------ >     Test of the model TF-IDF/Cos     /   Multi-classes & Mono-label')
        ...
        try:
            ...
            # Set model
            model_name = 'tfidf_cos_multi_class_mono_label'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
            test_model = model_tfidf_cos.ModelTfidfCos(x_col='preprocessed_text', y_col='y_col', level_save="HIGH",
                                                       tfidf_params={'ngram_range': (1, 2), 'min_df': 0.05, 'max_df': 0.9, 'binary': True, 'max_features': 100000},
    ...
    def test15_Model_TfidfSuperDocumentsNaive(self):
        '''Test of the model TF-IDF/Super Documents Naive'''
        print('            ------------------ >     Test of the model TF-IDF/Super Documents Naive     /   Multi-classes & Mono-label')
        ...
        try:
            ...
            # Set model
            model_name = 'tfidf_super_documents_naive_multi_class_mono_label'
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
            test_model = model_tfidf_super_documents_naive.ModelTfidfSuperDocumentsNaive(x_col='preprocessed_text', y_col='y_col', level_save="HIGH",
    ...
    def test16_Model_Aggregation(self):
        ...
        try:
            ...
            list_models = [model_tfidf_svm.ModelTfidfSvm(), model_tfidf_gbt.ModelTfidfGbt(), model_tfidf_gbt.ModelTfidfGbt()]
            model_dir = os.path.join(utils.get_models_path(), model_name, datetime.now().strftime(f"{model_name}_%Y_%m_%d-%H_%M_%S"))
            os.makedirs(model_dir)
            test_model = model_aggregation.ModelAggregation(x_col='preprocessed_text', y_col='y_col', level_save="HIGH",
                                                        list_models=list_models, using_proba=False, aggregation_function='majority_vote',
